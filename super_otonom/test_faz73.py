...
def test_faz73_missing_order_book_low_health_not_halt() -> None:
    ...
    r = detect_liquidity_games(symbol="BTC/USDT", analysis={"volatility": 0.03, "event_ts": 123}, order_book=None)
    d = r.to_dict()
    assert 0 <= d["manipulation_risk_score"] <= 100
    assert d["game_type"] in ("spoofing", "quote_stuffing", "momentum_ignition", "stop_hunt", "unknown", "none")
