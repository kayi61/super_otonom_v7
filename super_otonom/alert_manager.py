...
class AlertManager:
    ...
    def nav_diff(
        self, diff: float, diff_pct: float, local: float = 0.0, exchange: float = 0.0
    ) -> None:
    ...
    def _send(self, level: str, category: str, title: str, body: str) -> None:
        ...
        self._maybe_post_telegram(event)

        self._last_sent[category] = now

    def _maybe_post_telegram(self, event: AlertEvent) -> None:
        """TELEGRAM_BOT_TOKEN + TELEGRAM_CHAT_ID tanımlıysa kritik uyarıları Telegram'a gönder."""
        token = os.getenv("TELEGRAM_BOT_TOKEN", "").strip()
        chat_id = os.getenv("TELEGRAM_CHAT_ID", "").strip()
        if not token or not chat_id:
            return
        if event.level not in ("WARNING", "CRITICAL"):
            return
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=10) as resp:
                if resp.status >= 400:
                    log.warning("AlertManager | Telegram HTTP %s", resp.status)
        except Exception as exc:
            log.error("AlertManager | Telegram HATA | %s", exc)
    ...
    def snapshot(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent": [
                {"ts": e.ts, "level": e.level, "title": e.title, "sent": e.sent} for e in recent
            ],
