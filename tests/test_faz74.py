...
def test_faz74_no_venues_returns_unknown_low_confidence() -> None:
    ...
    r = infer_cross_venue_leadlag(
        symbol="BTC/USDT", analysis={"event_ts": 123}, half_life_ms=20_000
    )
    d = r.to_dict()
    assert d["leader_venue"] in ("unknown", "okx", "kucoin", "gate") or isinstance(
        d["leader_venue"], str
    )
    assert 0 <= d["leadlag_alpha_score"] <= 100
    assert 0 <= d["latency_arb_risk"] <= 100
    assert d["route_preference"] in (
        "leader",
        "best_price",
        "lowest_latency",
        "avoid_latency_arb",
