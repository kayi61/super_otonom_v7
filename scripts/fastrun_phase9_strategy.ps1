...
python -m super_otonom.edge_evidence --source synthetic --timeframe 5m --limit 400 --fee-bps 10 --slip-min 0.0002 --slip-max 0.0012 --exec-seed 42
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

if (-not $SkipPytest) {
    Write-Host ""
    Write-Host "pytest tests/test_edge_evidence_fastrun.py"
    python -m pytest tests/test_edge_evidence_fastrun.py tests/test_sharpe_annualize_fastrun.py -q --tb=short
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
}
