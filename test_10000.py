...
"""
import ast, json, math, os, sys, tempfile, time, types, logging, inspect
import importlib
...
cfg.RISK = {
    "max_daily_loss_pct": 0.03, "max_weekly_loss_pct": 0.10,
    "max_total_drawdown": 0.15, "max_exposure_pct": 0.95,
    "var_confidence": 0.95, "trailing_stop_pct": 0.02,
...
# Modülleri yükle
from capital_engine      import CapitalEngine, PositionLedger, JournalEntry
...
from pre_trade_gate      import (
    fat_finger_check, spread_check, ob_depth_check,
    same_bar_guard, merge_entry_notional,
    gate_buy_signal_and_slots, gate_global_trade_disable,
...
from stress_test         import StressTestRunner, SCENARIOS
...
def make_ce(cap=10000.0, reserve=0.0):
    t = tmp_path()
    return CapitalEngine(cap, journal_file=f"{t}/j.jsonl", reserve_pct=reserve, max_position_pct=1.0)

def make_ro(nav=10000.0): return RiskOntology(initial_nav=nav)
def make_rm(cap=10000.0): return RiskManager(initial_capital=cap)
def make_oe():
    t = tmp_path()
...
# 1-11: Syntax
for name, src in [("capital_engine",ce_src),("risk_ontology",ro_src),("risk_manager",rm_src),
                   ("order_engine",oe_src),("reconciliation_engine",re_src),("pre_trade_gate",ptg_src),
                   ("alert_manager",am_src),("market_impact",mi_src),("concentration_risk",cr_src),
                   ("stress_test",st_src),("audit_log",al_src)]:
    try: ast.parse(src); check(f"1.syntax.{name}", True)
    except SyntaxError as e: check(f"1.syntax.{name}", False, str(e))
...
for m in ["open_position","close_position","update_unrealized","record_fee",
          "deposit","withdrawal","reserve_margin","release_reservation",
          "snapshot","to_dict","from_dict","_check_invariant","_record",
          "get_journal","position_snapshot","all_positions"]:
...
for m in ["update","snapshot","to_dict","from_dict","is_daily_limit_breached",
          "is_weekly_limit_breached","is_drawdown_breached","is_exposure_breached",
          "_calc_var","_update_exposure","_maybe_reset_day","_maybe_reset_week"]:
    check(f"1.method.RO.{m}", f"def {m}(" in ro_src)

# 161-200: Metod varlığı — RiskManager
for m in ["check_risk","check_dynamic_risk","trigger_emergency","reset_emergency",
          "record_pnl","set_ontology","status_dict","calculate_var",
          "should_trailing_stop","record_volatility","check_volatility_spike",
          "_warn_if_onto_missing","get_last_deny"]:
    check(f"1.method.RM.{m}", f"def {m}(" in rm_src)

# 201-240: Metod varlığı — OrderEngine
for m in ["intent","sent","confirm","partial","fail","cancel",
          "is_duplicate","can_retry","recover","snapshot",
          "_write_log","_save_pending","_load_pending","pending_orders","failed_retryable"]:
    check(f"1.method.OE.{m}", f"def {m}(" in oe_src)

# 241-270: Metod varlığı — PreTradeGate
for f_name in ["gate_global_trade_disable","gate_buy_signal_and_slots",
               "merge_entry_notional","fat_finger_check","spread_check",
               "ob_depth_check","same_bar_guard","gate_buy_size_and_exposure"]:
    check(f"1.func.PTG.{f_name}", f"def {f_name}(" in ptg_src)

# 271-300: Metod varlığı — AlertManager
for m in ["emergency","nav_diff","circuit_breaker","stale_data",
          "backoff","system","tca_anomaly","snapshot","_send","_post_webhook"]:
...
for m in ["trade_open","trade_close","risk_block","emergency",
          "signal_event","system_event","get_events","today_summary"]:
...
# Multiple ops
e = make_ce(10000)
for _ in range(5): e.deposit(100)
for _ in range(3): e.withdrawal(50)
...
# to_dict keys
d = e.to_dict()
for key in ["initial_capital","cash","margin_used","reserved_margin",
            "unrealized_pnl","realized_pnl","fees_paid","net_deposits",
            "reserve_pct","max_position_pct","positions"]:
...
e.open_position("BTC/USDT", "o1", 50000, 0.1, 5000)
snap = e.snapshot()
for k in ["nav","cash","margin_used","reserved_margin","unrealized_pnl",
          "realized_pnl","fees_paid","net_deposits","available_cash",
          "buying_power","open_positions","total_return_pct","journal_entries"]:
...
check("2.CE.multi_pos.margin_sum",      abs(e._margin_used - sum(1000*(i+1) for i in range(5))) < 0.01)
...
snap = ro.snapshot()
for k in ["nav","sod_nav","sow_nav","peak_nav","intraday_dd_pct",
          "daily_loss_pct","weekly_loss_pct","dynamic_daily_limit",
...
with tempfile.TemporaryDirectory() as _tmp46:
    ...
    _oe46.sent(_ids46[0]); _oe46.confirm(_ids46[0], 0.1, 100)
...
def make_recon(nav=10000.0):
    cap = MagicMock()
    cap.nav = nav; cap._cash = nav; cap._margin_used = 0.0
    cap._unrealized_pnl = 0.0; cap._positions = {}
    cap._record = MagicMock(); cap._reserved_margin = 0.0
    oe_m = MagicMock(); oe_m.recover = AsyncMock(return_value=[])
    t = tmp_path()
    return ReconciliationEngine(capital=cap, order_engine=oe_m,
                                recon_dir=t, tolerance_pct=0.02, hard_block_pct=0.10), cap
...
# ReconResult fields
check("4.RE.result.nav_fields",         all(hasattr(result,f) for f in
    ["local_nav","exchange_nav","nav_diff","nav_diff_pct","nav_ok","hard_blocked"]))
check("4.RE.result.position_fields",    all(hasattr(result,f) for f in
    ["local_positions","exchange_positions","position_mismatch"]))
...
check("6.ST.sideways_survived",         results.get("SIDEWAYS_LOW_VOL", type('x',(),{"survived":False})()).survived)

# 6.6: ConcentrationRisk + CapitalEngine (2741-2800)
cr = ConcentrationRiskManager()
e = make_ce(10000)
e.open_position("BTC/USDT", "o1", 50000, 0.1, 5000)
ok, _ = cr.check_concentration("BTC/USDT", 1000, 10000, e.open_positions if hasattr(e, 'open_positions') else {})
...
e.open_position("BTC/USDT", "o1", 50000, 0.1, 5000)
d = e.to_dict()
required_keys = ["initial_capital","cash","margin_used","reserved_margin",
                 "unrealized_pnl","realized_pnl","fees_paid","net_deposits",
                 "reserve_pct","max_position_pct","positions"]
...
for trial in range(100):
    ...
    for _ in range(n_ops):
        ...
        if op == "open" and len(e._positions) < 5:
            ...
            if notional > 0:
                e.open_position(f"S{trial}", f"o{trial}_{_}",
                                rng.uniform(100, 50000), rng.uniform(0.01, 1.0), notional)
        elif op == "close" and e._positions:
            sym = rng.choice(list(e._positions.keys()))
            e.close_position(sym, f"c{trial}_{_}",
                            rng.uniform(100, 50000), e._positions[sym].qty * rng.uniform(0.5, 1.0))
        elif op == "deposit":
...
for trial in range(50):
    ...
    ids = [oe.intent(f"S{rng.randint(0,10)}", "BUY", rng.uniform(0.01, 1), rng.uniform(100, 10000))
           for _ in range(n)]
...
for trial in range(50):
    ...
    for _ in range(rng.randint(1, 5)):
        ...
        if notional > 0 and len(e._positions) < 3:
            e.open_position(f"S{_}", f"o{trial}_{_}",
                           rng.uniform(100, 10000), 1.0, notional)
...
valid_transitions = {
    ...
    OrderState.SENT:      [OrderState.FILLED, OrderState.PARTIAL, OrderState.FAILED, OrderState.CANCELLED],
...
for trial in range(40):
    ...
    check(f"10.prop.MI.bounds{trial}",
          0.0001 <= est.total_pct <= 0.02,
          f"total_pct={est.total_pct}")
...
e = CapitalEngine(10000, journal_file=f"{tmp_path()}/j.jsonl", reserve_pct=0.0, max_position_pct=0.5)
check("11.CE.bp.50pct",                 abs(e.buying_power - 5000) < 0.01)
e2 = CapitalEngine(10000, journal_file=f"{tmp_path()}/j.jsonl", reserve_pct=0.0, max_position_pct=0.8)
...
rm = make_rm(10000)
st = rm.status_dict()
for k in ["daily_loss","weekly_loss","var_95","emergency_stop",
          "emergency_reason","last_risk_deny","peak_equity"]:
...
oe.sent(oid1); oe.confirm(oid1, 0.1, 100)
oe.fail(oid2, "err")
snap = oe.snapshot()
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    for i in range(15):
        ...
        oe.sent(oid); oe.confirm(oid, 0.1, 100)
    # PENDING'ler silinmemeli
    for pid in pending_ids[:5]:
        rec = oe.get(pid)
        check(f"13.OE.mem.pending_kept", rec is not None and rec.state == OrderState.PENDING)
...
check("15.AM.emergency.title",          "EMERGENCY" in am._history[-1].title or "code1" in am._history[-1].title)
...
# snapshot
mi_snap = mi.snapshot()
check("16.MI.snap.has_keys",           all(k in mi_snap for k in
    ["total_estimates","large_orders","avg_impact_pct","lambda"]))
...
check("17.CR.snap.keys",               all(k in cr_snap for k in
    ["sector_breakdown","concentration_score","max_sector_pct"]))
...
for cap in [1000, 10000, 100000, 1000000]:
    r2 = StressTestRunner(capital=cap).run_scenario("FLASH_CRASH_RECOVERY",
         SCENARIOS["FLASH_CRASH_RECOVERY"])
...
t = tmp_path()
al = AuditLog(audit_dir=t)
al.trade_open("BTC/USDT", "o1", 50000, 0.1, 5000,
              fee=5.0, confidence=0.75, nav=10000, cash=5000, open_positions=1)
events = al.get_events()
...
al.trade_close("BTC/USDT", "o2", 52000, 0.1, pnl=200, fee=5.0,
               reason="SELL_SIGNAL", nav=10195, realized_pnl=200)
...
check("20.DR.open_pos.warning",         any("açık" in w.lower() or "open" in w.lower() for w in report3.warnings))
    ...
    for _ in range(_rng22.randint(1, 15)):
        ...
        try:
            if op == "open" and len(e._positions) < 5 and e.available_cash > 10:
                ...
                e.open_position(f"S{trial}_{_}", f"o{trial}_{_}", _rng22.uniform(10, 60000), max(0.001, n/_rng22.uniform(100,60000)), n)
            elif op == "close" and e._positions:
                sym = _rng22.choice(list(e._positions.keys()))
                e.close_position(sym, f"c{trial}_{_}", _rng22.uniform(1,60000), e._positions[sym].qty*_rng22.uniform(0.3,1.0))
            elif op == "dep": e.deposit(_rng22.uniform(1, 500))
            elif op == "with" and e.available_cash > 5: e.withdrawal(_rng22.uniform(1, min(e.available_cash*0.05, 50)))
            elif op == "fee": e.record_fee("X", f"f{trial}_{_}", _rng22.uniform(0.01, 5))
            elif op == "unreal" and e._positions: e.update_unrealized({s: _rng22.uniform(1, 70000) for s in e._positions})
            elif op == "reserve" and e.available_cash > 10: e.reserve_margin(f"r{trial}_{_}", _rng22.uniform(1, min(e.available_cash*0.2, 500)))
            elif op == "release" and e._reserved_margin > 0: e.release_reservation(f"r{trial}", min(e._reserved_margin, _rng22.uniform(1, 100)))
        except Exception:
            pass
...
for trial in range(50):
    ...
    for _ in range(20): ro.update(nav=_rng22.uniform(100, 200000))
    check(f"23.peak.{trial}", ro.peak_nav >= ro.nav)

for trial in range(50):
    ro = make_ro(_rng22.uniform(1000, 100000))
    n0 = _rng22.uniform(5000, 50000); ro.update(nav=n0)
    for _ in range(10): ro.update(nav=_rng22.uniform(n0*0.3, n0*1.5))
    check(f"23.loss_nn.{trial}", ro.daily_loss_pct >= 0.0)

for trial in range(50):
    ro = make_ro(10000)
    for _ in range(10): ro.update(nav=_rng22.uniform(5000, 15000))
...
for trial in range(50):
    rm = make_rm(10000); tl = 0
    for _ in range(_rng22.randint(1,20)):
        p = _rng22.uniform(-500, 500); rm.record_pnl(p)
        if p < 0: tl += abs(p)
...
for trial in range(50):
    rm = make_rm(); ro = make_ro(); rm.set_ontology(ro)
    for i in range(1, _rng22.randint(50, 150)): rm.record_pnl(_rng22.uniform(-200, 200))
...
for trial in range(100):
    oe = make_oe()
    ids = [oe.intent(f"S{_rng22.randint(0,10)}", "BUY", 0.1, 100) for _ in range(_rng22.randint(5,50))]
...
for trial in range(100):
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for a in [_rng22.choice(["sent","confirm","fail","cancel"]) for _ in range(3)]:
            try:
                prev = oe.get(oid).state
                if a == "sent" and prev == OrderState.PENDING: oe.sent(oid)
                elif a == "confirm" and prev in (OrderState.SENT, OrderState.PARTIAL): oe.confirm(oid, 0.1, 100)
                elif a == "fail": oe.fail(oid, "err")
                elif a == "cancel" and prev not in (OrderState.FILLED,): oe.cancel(oid)
...
for trial in range(60):
    ts = _rng22.uniform(1000, 2000000); sym = f"SYM{trial}/USDT"
...
for trial in range(100):
    est = mi26.estimate(_rng22.uniform(1, 1000000), _rng22.uniform(100, 10000000), _rng22.uniform(0.001, 0.5))
...
for trial in range(50):
    ...
    check(f"26.hhi.{trial}", 0.0 <= cr26.concentration_score(pos, _rng22.uniform(10000, 100000)) <= 1.0)

for trial in range(50):
    est = mi26.estimate(_rng22.uniform(100, 10000), _rng22.uniform(10000, 1000000), _rng22.uniform(0.005, 0.1))
...
for trial in range(50):
    e = make_ce(_rng22.uniform(1000, 50000))
    nb = e.nav; e.deposit(_rng22.uniform(1, 10000))
    check(f"28.dep_inc.{trial}", e.nav > nb)

for trial in range(50):
    cap = _rng22.uniform(1000, 50000); e = make_ce(cap)
    nb = e.nav; result = e.withdrawal(_rng22.uniform(1, cap*2))
...
for i in range(250): am29._send("INFO", f"CAT_{i}", f"t_{i}", "b")
...
if _CPP_PATH:
    ...
    try:
        from risk_engine import check_risk_cpp, check_dynamic_risk_cpp, should_trailing_stop_cpp, calculate_risk_score
        _HAS_RISK_ENGINE = True
    except ImportError:
        pass
    try:
        from var_engine import calc_var, amihud_ratio, estimate_impact, rolling_mean, rolling_std
...
if _HAS_RISK_ENGINE:
    ...
    check("31.cpp.dyn.clamp_low", check_dynamic_risk_cpp(150, 10000, 0.001) is True)  # limit=2%, 1.5%<2%
    check("31.cpp.dyn.clamp_high", check_dynamic_risk_cpp(400, 10000, 0.10) is True)  # limit=5%, 4%<5%
...
for trial in range(100):
    est = mi35.estimate(
        _rng22.uniform(1, 500000),
        _rng22.uniform(100, 5000000),
        _rng22.uniform(0.001, 0.3)
    )
...
for trial in range(50):
    est = mi38.estimate(_rng22.uniform(100, 50000), _rng22.uniform(10000, 5000000), _rng22.uniform(0.005, 0.1))
    p = _rng22.uniform(100, 60000)
...
for trial in range(100):
    ...
    pos = {s: {"size": _rng22.uniform(100, 5000), "notional": _rng22.uniform(100, 5000)} for s in syms}
...
for trial in range(100):
    ...
    ok, reason = cr.check_concentration(
        f"S{trial}/USDT",
        _rng22.uniform(100, 5000),
        _rng22.uniform(5000, 50000),
        {}
    )
...
for trial in range(100):
    ...
    report = dr.run(snap)
    check(f"49.DR.win_lose.{trial}",
          report.winning_trades == n_win and report.losing_trades == n_lose)
...
for trial in range(100):
    ...
    check(f"49.DR.reset.{trial}",
          len(dr._trade_log) == 0 and abs(dr._sod_nav - new_nav) < 0.01)
...
for trial in range(100):
    ...
    check(f"51.FAIL.release.{trial}", abs(e.available_cash - e.nav + e._margin_used + e._reserved_margin) < 0.01)
    ...
    for _ in range(_rng22.randint(0, 8)):
        ...
        try:
            if op == "open" and len(e._positions) < 4 and e.available_cash > 10:
                n = _rng22.uniform(1, min(e.available_cash*0.3, 3000))
                e.open_position(f"P{trial}_{_}", f"o{_}", _rng22.uniform(10,60000), max(0.001, n/max(1,_rng22.uniform(10,60000))), n)
            elif op == "close" and e._positions:
                sym = _rng22.choice(list(e._positions.keys()))
                e.close_position(sym, f"c{_}", _rng22.uniform(1,60000), e._positions[sym].qty*_rng22.uniform(0.3,1.0))
            elif op == "dep": e.deposit(_rng22.uniform(1,500))
            elif op == "fee": e.record_fee("X","f",_rng22.uniform(0.01,5))
            elif op == "unreal" and e._positions:
    ...
    for _ in range(_rng22.randint(1, 10)):
        try:
            ...
            if op == "open" and len(e._positions) < 5 and e.available_cash > 5:
                ...
                e.open_position(f"S{trial}_{_}",f"o{trial}_{_}",_rng22.uniform(10,50000),max(0.001,n/_rng22.uniform(10,50000)),n)
            elif op == "close" and e._positions:
                s = _rng22.choice(list(e._positions.keys()))
                e.close_position(s,f"c{trial}_{_}",_rng22.uniform(1,60000),e._positions[s].qty*_rng22.uniform(0.5,1.0))
            elif op == "dep": e.deposit(_rng22.uniform(1,300))
            elif op == "with" and e.available_cash > 2: e.withdrawal(_rng22.uniform(1,min(e.available_cash*0.05,50)))
            elif op == "fee": e.record_fee("X",f"f{_}",_rng22.uniform(0.01,3))
            elif op == "unreal" and e._positions: e.update_unrealized({s:_rng22.uniform(1,60000) for s in e._positions})
            elif op == "res" and e.available_cash > 5: e.reserve_margin(f"r{_}",_rng22.uniform(1,min(e.available_cash*0.1,200)))
            elif op == "rel" and e._reserved_margin > 0: e.release_reservation(f"r{_}",min(e._reserved_margin,_rng22.uniform(1,100)))
        except Exception: pass
...
for trial in range(500):
    ...
    ids = [oe.intent(f"S{i%10}", _rng22.choice(["BUY","SELL"]), _rng22.uniform(0.01,5), _rng22.uniform(10,100000)) for i in range(n)]
...
for trial in range(100):
    rm = make_rm(10000)
    rm.trigger_emergency(f"first_{trial}")
    for _ in range(5): rm.trigger_emergency(f"other_{trial}_{_}")
...
for trial in range(100):
    est = mi59.estimate(_rng22.uniform(1,1000000), _rng22.uniform(100,10000000), _rng22.uniform(0.001,0.5))
...
for trial in range(100):
    est = mi59.estimate(_rng22.uniform(100,10000), _rng22.uniform(10000,1000000), _rng22.uniform(0.005,0.1))
    p = _rng22.uniform(100,60000)
...
for trial in range(200):
    ...
    try:
        if action == "sent": oe.sent(oid)
        elif action == "confirm": oe.sent(oid); oe.confirm(oid,0.1,100)
        elif action == "fail": oe.fail(oid,"err")
        elif action == "cancel": oe.cancel(oid)
        elif action == "partial": oe.sent(oid); oe.partial(oid,0.05,100)
    except Exception: pass
